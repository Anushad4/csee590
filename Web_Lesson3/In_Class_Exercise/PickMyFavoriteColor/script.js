...
function clickFavButton(){
    // to retreve the color to preview display
    var input = document.getElementById("color");
    color = input;
    // call the setpreviewcolor
...
$(document).ready(function(){
    ...
    for( x = 0; x<colors.length;x++) {
        $(".grid-item").each(function (x) {
       //adding color to grid item accordingly
...
// the content of the input gets cleared and the focus gets back on the input
    ...
    $('#add-to-favorite').click( function(){
        //to retrieve the color given in input
        color = document.getElementById("color").value;
        //push method is used to dump the color
        colors.push(color);
        //to empty the text given in input text box
        $('#color').val("");
        //to focus the cursor in input text box for next input
        $('#color').focus();
        // $( ".grid-container" ).append( "<div class="grid-item"></div>" );
        // after()
        //to add a grid to the last grid in the container
        $(".grid-container" ).append(" <div class=\"grid-item\"></div>");
...
$(".grid-item") .hover(function(){
    //to display the color  in set preview by clicking a grid item correspondingly
